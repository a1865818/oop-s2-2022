...
void Inventory::pick(Item theItem)
{
...
void Inventory::use(Item item)
{
	for (int i = 0; i < itemNumber; i++)
	{
		...
		if (item.getName() == item_list[i].getName())
		{
...
int Inventory::getTotalValue()
{
	...
	if (itemNumber > 0)
	{
		// use for loop to iterate every element and add the value to the variable totalValue
		for (int i = 0; i < itemNumber; i++)
		{
			totalValue += item_list[i].getValue();
		}
	}
	return totalValue;
}
