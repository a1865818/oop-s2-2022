...
Game::Game()
{
	...
	// Game description
	cout << "                                    -------------------------------------------" << endl;
	cout << "                                    | Harry Potter and the Chamber of Secrets |" << endl;
	cout << "                                    -------------------------------------------" << endl;
	cout << " There are 9 secret rooms in Hogwarts. You are Harry Potter, in the first room, which is the entrance. And your\n mission is to rescue Hermione and Ron who are currently trapped in the last room, which is room 9. These rooms\n will be arranged as a 3x3 matrix and have some items and Monsters located in certain fixed rooms. You will have\n an inventory to keep the items you collect during the adventure, each item will have its own value. The sword \n of Gryffindor in room 3 will help you kill the Basilisk reptile in room 8 while the Elder wand in room 7 will\n help you defeat the Dementor guard in room 6. With simple given commands on the screen , you will have chances\n to overcome obstacles, and once reach room number 9, you will successfully rescue your friends and be the winner!" << endl;
	string input;
...
void Game::attack()
{
	...
	string monsterName = roomList[currentRoomNumber - 1]->getCurrentMonster()->getName();
	bool monsterStatus = roomList[currentRoomNumber - 1]->getCurrentMonster()->getStatus();
	// if the monster is alive
	if (monsterStatus)
	{
		// get the list of items in the inventory of the player
		Inventory *invent;
		invent = currentPlayer->getInventory();
		Item *itemList;
		...
		for (int i = 0; i < 7; i++)
		{
			if (roomList[currentRoomNumber - 1]->getCurrentMonster()->isDead(itemList[i]))
			{
				check = true;
				cout << "You have killed " << monsterName << " successfully!" << endl;
				roomList[currentRoomNumber - 1]->getCurrentMonster()->setStatus(false);
			}
		}
...
void Game::pick()
{
	...
	if (roomList[currentRoomNumber - 1]->getCurrentItem()->getValue() != 0)
	{
		Inventory *invent;
		invent = currentPlayer->getInventory();
		// pick up the item
		invent->pick(*roomList[currentRoomNumber - 1]->getCurrentItem());
		currentPlayer->setInventory(invent);
		// announce the result
		cout << "Stored " << roomList[currentRoomNumber - 1]->getCurrentItem()->getName() << " successfully!" << endl;
		roomList[currentRoomNumber - 1]->getCurrentItem()->setName("");
		roomList[currentRoomNumber - 1]->getCurrentItem()->setValue(0);
	}
...
void Game::showLocation()
{
	...
	cout << endl
		 << currentPlayer->getName() << ", you are in Room " << currentRoomNumber << "." << endl;
	// display item and monster in the room
	if (roomList[currentRoomNumber - 1]->getCurrentItem()->getValue() != 0)
	{
		cout << "There is " << roomList[currentRoomNumber - 1]->getCurrentItem()->getName() << " in this room!" << endl;
	}
	else
	{
	...
	if (roomList[currentRoomNumber - 1]->getCurrentMonster()->getStatus())
	{
		cout << "WATCH OUT!" << endl;
		roomList[currentRoomNumber - 1]->getCurrentMonster()->display();
		currentPlayer->display();
		roomList[currentRoomNumber - 1]->getCurrentMonster()->showDetails();
	}
	else
	{
...
void Game::move()
{
	...
	while (true)
	{
		...
		{
			transformMove = "East";
			if (currentRoomNumber == 8 && roomList[currentRoomNumber - 1]->getCurrentMonster()->getStatus() == true)
			{
				lose();
			}
			for (int i = 0; i < numOfDoors; i++)
		...
		{
			transformMove = "South";
			if (currentRoomNumber == 6 && roomList[currentRoomNumber - 1]->getCurrentMonster()->getStatus() == true)
			{
				lose();
			}
			for (int i = 0; i < numOfDoors; i++)
...
void Game::win()
{
	...
	cout << "Congratulation! Hermione and Ron are here and still safe! You are the winner!" << endl;
	isDone = true;
}
...
void Game::lose()
{
	...
	cout << "Your are killed by " << roomList[currentRoomNumber - 1]->getCurrentMonster()->getName() << "! You lose!" << endl;
	isDone = true;
}
